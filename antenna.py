...
class draw_picture():
    ...
    def visualization(self,data,time,name,picturedir):#1根天线
        ...
        for i in range(1,len(time)):
            time_list.append(int(time[i])-int(time[i-1])+time_list[i-1])
...
# 多进程
def distinguish_analyse(path):
    start=time.time()
    if path[1]==1:
        A1 = one_antenna()
        A1.distribute_group(path[0])
    if path[1]==2:
        A2 = double_antenna()
        A2.distribute_group(path[0])
    end=time.time()
    print(end-start)

if __name__ == '__main__':
    pathbox=[
        (r'1antenna_test\recorddata_out1.csv', 1),
        (r'1antenna_test\recorddata_out2.csv', 1),
        (r'2antenna_test\recorddata_out1.csv', 2),
    ]
    p = Pool(4)
    for i in range(len(pathbox)):
        p.apply_async(distinguish_analyse,args=(pathbox[i],))
